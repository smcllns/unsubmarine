var app = (function () {
    ...
    function isListView() {
      ...
      return Array.from(document.querySelectorAll("[role=main] [role=row]")).filter(function (node) {
    ...
    function scrapeListMeta() {
      ...
      var possibleListItems = Array.from(document.querySelectorAll("[role=main] [role=row]")).filter(function (node) {
        return node.offsetParent;
      });
      var firstItem = possibleListItems[0];
      var listLength = possibleListItems.length;
      var possibleNextBtns =  document.querySelectorAll('[data-tooltip="Older"]') ;
      var nextBtn = Array.from(possibleNextBtns).filter(function (node) {
        return node.offsetParent;
      })[0]; // On narrow viewport width, gmail can hide these buttons, in that case, choose the one furthest down the page (likely highest on UI layers?)

      if (!nextBtn && possibleNextBtns.length > 0) nextBtn = possibleNextBtns[possibleNextBtns.length - 1];
      return {
        firstItem: firstItem,
    ...
    function scrapeMessageMeta() {
      var _possibleUnsubBtn$tex, _document$querySelect, _document$querySelect2, _document$querySelect3;

      var possibleUnsubLinks = Array.from(document.querySelectorAll('[role="listitem"] a')).filter(function (a) {
        return /unsubscribe/i.test(a === null || a === void 0 ? void 0 : a.textContent);
      });
      var unsubLink = possibleUnsubLinks.length > 0 ? possibleUnsubLinks[0] : false;
      var possibleUnsubBtn = document.querySelector("[idlink]");
      var unsubBtn = (possibleUnsubBtn === null || possibleUnsubBtn === void 0 ? void 0 : (_possibleUnsubBtn$tex = possibleUnsubBtn.textContent) === null || _possibleUnsubBtn$tex === void 0 ? void 0 : _possibleUnsubBtn$tex.toLowerCase()) === "unsubscribe" ? possibleUnsubBtn : false;
      var possibleNextBtns =  document.querySelectorAll('[data-tooltip="Older"]') ;
      var nextBtn = Array.from(possibleNextBtns).filter(function (node) {
        return node.offsetParent;
      })[0]; // On narrow viewport width, gmail can hide these buttons, in that case, choose the one furthest down the page (likely highest on UI layers?)
      ...
      if (!nextBtn && possibleNextBtns.length > 0) nextBtn = possibleNextBtns[possibleNextBtns.length - 1];
      var sender = (_document$querySelect = document.querySelectorAll("span .go")[0]) === null || _document$querySelect === void 0 ? void 0 : _document$querySelect.textContent;
      var subject = (_document$querySelect2 = document.querySelectorAll("[data-thread-perm-id]")[0]) === null || _document$querySelect2 === void 0 ? void 0 : _document$querySelect2.textContent;
      var when = (_document$querySelect3 = document.querySelectorAll('[alt*=":"][title*=":"]')[0]) === null || _document$querySelect3 === void 0 ? void 0 : _document$querySelect3.textContent;
